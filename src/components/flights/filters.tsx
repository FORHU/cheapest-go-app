...
export default function FlightFilters({ airlines, onFilterChange }: FlightFiltersProps) {
    ...
    return (
        <div className="flex flex-col gap-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm self-start w-full lg:w-72">
            {/* Sorting */}
            <div className="space-y-3">
                ...
                <div className="flex flex-col gap-2">
                    <button 
                        ...
                        className={`text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${state.sortBy === 'price' ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                        Cheapest First
                    </button>
                    <button 
                        onClick={() => updateSort("duration")}
                        className={`text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${state.sortBy === 'duration' ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                        Fastest First
                    </button>
                    <button 
                        onClick={() => updateSort("departure")}
                        className={`text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${state.sortBy === 'departure' ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                        Earliest Departure
                    </button>
                </div>
                ...
                <div className="flex flex-col gap-2">
                    {[
                        ...
                        <button 
                            ...
                            className={`text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${state.maxStops === option.value ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
                            {option.label}
                        </button>
                    ))}
