...
export function Sidebar({ onClose }: SidebarProps) {
                ...
                {items.map((item) => {
                    ...
                    return (
                        <Link key={item.href} href={item.href} onClick={onClose}>
                            <motion.div
                                className={`relative group flex items-center gap-3 px-4 py-3 cursor-pointer transition-all duration-300 ${isActive
                                    ? 'text-white'
    ...
    return (
        <aside className="w-72 h-screen flex flex-col bg-slate-100 dark:bg-obsidian border-r border-slate-100 dark:border-white/5 relative z-50">
            {/* Logo Section */}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-10 thin-scrollbar">
                <NavigationGroup title="Menu" items={navItems.slice(0, 4)} />
                <NavigationGroup title="General" items={navItems.slice(4)} />

                {/* Sidebar Widget (Download App) */}
