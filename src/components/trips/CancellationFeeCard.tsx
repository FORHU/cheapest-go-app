...
export function CancellationFeeCard({ feeResult }: CancellationFeeCardProps) {
    const t = useTranslations('trips.cancellationFeeCard');
    return (
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 mb-5">
            ...
            <div className="grid grid-cols-2 gap-4 mt-3">
                <div>
                    <p className="text-xs text-blue-600 dark:text-blue-400 mb-1">{t('fee')}</p>
                    <p className="text-lg font-bold text-blue-900 dark:text-blue-100">
                        {formatCurrency(feeResult.fee, feeResult.currency)}
                    </p>
                </div>
                <div>
                    <p className="text-xs text-green-600 dark:text-green-400 mb-1">{t('youllReceive')}</p>
