...
const MODE_LABELS = {
    ground:    { icon: Navigation, color: 'text-emerald-400', label: 'Navigate' },
    regional:  { icon: MapPin,     color: 'text-amber-400',   label: 'Too far to drive directly' },
    air:       { icon: Plane,      color: 'text-blue-400',    label: 'Flying in' },
    resolving: { icon: Loader2,    color: 'text-slate-400',   label: 'Locating…' },
};
...
export default function TripMapView({ booking, onClose }: TripMapViewProps) {
    ...
    // 2. Geocode hotel
    const { coords: hotelCoords, isLoading: isGeocoding } = useHotelGeocoder(booking.property_name);
    ...
    const initialViewState = hotelCoords
        ? { longitude: hotelCoords.lng, latitude: hotelCoords.lat, zoom: mode === 'ground' ? 13 : 11, pitch: 20, bearing: 0 }
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col bg-slate-950">
            ...
            {/* ── Header ── */}
            <div className="relative z-10 flex items-center gap-3 px-4 py-3 bg-slate-900/95 backdrop-blur-md border-b border-white/10">
                <button
                    onClick={onClose}
            ...
            <div className="flex-1 relative">
                ...
                <div className="absolute top-3 right-3 z-10 flex flex-col gap-2">
                    <button
                        ...
                        className="p-2 rounded-xl bg-slate-900/90 backdrop-blur-md border border-white/10 text-white hover:bg-white/10 disabled:opacity-40 transition-all shadow-lg"
                    ...
                    <button
                        ...
                        className={`p-2 rounded-xl backdrop-blur-md border transition-all shadow-lg ${
                            trafficOn
                                ...
                                : 'bg-slate-900/90 border-white/10 text-white hover:bg-white/10'
                        }`}
            ...
            {/* ── Bottom HUD ── */}
            <div className="relative z-10 bg-slate-900/95 backdrop-blur-md border-t border-white/10 px-4 py-4">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex-1 grid grid-cols-2 gap-3">
                            <div className="bg-white/5 rounded-xl px-3 py-2">
                                <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider mb-0.5">Driving</p>
                                <p className="text-white font-bold text-lg leading-tight">
                                    {isFetchingRoute ? <Loader2 className="w-4 h-4 animate-spin" /> : formatMins(travelTime)}
                                </p>
                                {distanceKm !== null && (
                                    <p className="text-slate-400 text-xs">{formatKm(distanceKm)}</p>
                                )}
                            </div>
                            <div className="bg-white/5 rounded-xl px-3 py-2">
                                <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider mb-0.5">Walking</p>
                                <p className="text-white font-bold text-lg leading-tight">
                                    {isFetchingRoute ? <Loader2 className="w-4 h-4 animate-spin" /> : formatMins(walkingTime)}
                                </p>
                            </div>
                        </div>
                        <button
                            ...
                            disabled={!hotelCoords}
                            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white font-semibold text-sm px-5 py-3 rounded-xl transition-colors shadow-lg shadow-blue-900/40"
                        >
                ...
                {mode === 'regional' && (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-slate-400 text-xs">Navigation starts when you're within 50 km</p>
                        </div>
                        <button
                            onClick={openGoogleMaps}
                            disabled={!hotelCoords}
                            className="text-blue-400 text-xs font-medium border border-blue-500/30 rounded-lg px-3 py-1.5 hover:bg-blue-500/10 transition-colors disabled:opacity-40"
                        >
                            Open in Maps
                        </button>
                    </div>
                ...
                {mode === 'air' && (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            <p className="text-white font-semibold text-sm">Flying in</p>
