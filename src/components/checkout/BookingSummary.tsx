...
function getRatingLabel(score: number, t: (key: string) => string): string {
    if (score >= 9) return t('exceptional');
    if (score >= 8) return t('fabulous');
        ...
        <div className="lg:col-span-1">
            <div className="bg-white dark:bg-slate-900 rounded-lg lg:rounded-xl border border-slate-200 dark:border-white/10 shadow-lg sticky top-24 overflow-hidden">
                ...
                <div className="px-3 lg:px-5 pb-3 lg:pb-5 space-y-3 lg:space-y-4">
                            ...
                            {editingDates ? (
                                <div className="space-y-2 mt-1">
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-0.5">{t('checkInLabel')}</p>
                                            <input
                                                type="date"
                                                value={draftCheckIn}
                                                min={today}
                                        ...
                                        <div>
                                            <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-0.5">{t('checkOutLabel')}</p>
                                            <input
                                                type="date"
                                                value={draftCheckOut}
                                ...
                                <div className="text-[11px] lg:text-sm font-medium text-slate-900 dark:text-white">
                                    ...
                                    <span className="text-slate-500 dark:text-slate-400 font-normal ml-1">
                                        ({totalNights} {t(totalNights === 1 ? 'night' : 'nights')})
                                    </span>
                                </div>
                            )}
                        </div>
                    ...
                    {/* Your Room */}
                    <div className="border-t border-dashed border-slate-200 dark:border-white/10 pt-3 lg:pt-4">
                        <div className="text-[10px] lg:text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5 lg:mb-2">{t('yourRoom')}</div>
                        ...
                        <div className={`${breakdownOpen ? 'block' : 'hidden'} lg:block space-y-1.5 lg:space-y-2`}>
                            <div className="flex justify-between text-[11px] lg:text-sm">
                                <span className="text-slate-600 dark:text-slate-400">
                                    {t('roomNights', { nights: totalNights, count: t(totalNights === 1 ? 'night' : 'nights') })}
                                </span>
