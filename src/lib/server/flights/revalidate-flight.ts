 ...
 *
 * This was `functions/v1/revalidate-flight` on Supabase, then `/api/internal/revalidate-flight`
 * once the Deno functions became API routes. Both callers live in this same process, so the
 * HTTP hop between them was an artefact of where the code used to run — it cost a loopback
 * request, a `FUNCTIONS_SECRET` round-trip and a JSON re-serialisation on the critical path
 * between quoting a fare and charging for it, and it could be misrouted to a different
 * instance entirely whenever `NEXT_PUBLIC_SITE_URL` did not match the port actually served.
 *
 * The route still exists as a thin wrapper for anything calling it over HTTP.
 *
 * - Mystifly: `/api/AirRevalidate` with the FareSourceCode
 * - Duffel:   the Price Action (`/air/offers/{id}/actions/price`)
 */
...
export interface RevalidateFlightInput {
    provider?: string;
    flightPayload?: any;
    userId?: string;
}

export interface RevalidateFlightResult {
    success: boolean;
    ...
    farePolicy?: any;
    error?: string;
    /** Set when the input itself was wrong, so the HTTP wrapper can answer 400 rather than 200. */
    badRequest?: boolean;
}
...
export async function revalidateFlight(input: RevalidateFlightInput): Promise<RevalidateFlightResult> {
...
async function revalidateMystifly(flight: any): Promise<RevalidateFlightResult> {
    ...
    if (!fareSourceCode) {
        return { success: false, seatsAvailable: false, error: 'No FareSourceCode in flight data' };
    }

    if (!searchIdentifier) {
        console.warn(`[revalidate-flight] Mystifly: No SearchIdentifier for FSC ${fareSourceCode.slice(0, 16)} — cannot revalidate`);
        return { success: false, seatsAvailable: false, error: 'SearchIdentifier is empty — cannot revalidate this fare' };
    }
    ...
    try {
        ...
        return { success: false, seatsAvailable: false, error: err.message };
    }

    if (!revalData?.Success) {
        const errMsg = revalData?.Message ?? 'Revalidation failed';
        return { success: false, seatsAvailable: false, error: errMsg };
    }
...
async function revalidateDuffel(flight: any): Promise<RevalidateFlightResult> {
    const duffelToken = env.DUFFEL_TOKEN;
    if (!duffelToken) {
        return { success: false, seatsAvailable: false, error: 'Duffel not configured' };
    }
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if (res.status === 404) {
                return { success: false, seatsAvailable: false, error: 'Offer no longer available' };
            }
