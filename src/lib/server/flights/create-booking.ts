/**
 * Turn a paid booking session into a flight booking.
 *
 ...
 *
 * Was `functions/v1/create-booking` on Supabase, then `/api/internal/create-booking`. Every
 * caller — the Stripe webhook, /flights/confirm, the mobile confirm route and admin
 * recovery — runs in this same process, so the loopback HTTP request between them was an
 * artefact of where the code used to live. Worse, it resolved through
 * NEXT_PUBLIC_SITE_URL, so a stale or mismatched value pointed the call at a different
 * instance and the booking silently failed to be created after the customer had paid.
 * See ADR-0012. The route remains as a thin HTTP wrapper.
 *
 * The logic below is unchanged from the route it came from, including its use of
 * NextResponse as the return type: `createBooking()` unwraps that in-process. The
 * remaining serialisation is an in-memory object round-trip, not a network call, and
 * rewriting 572 lines of payment-capture logic to avoid it would risk far more than it
 * saves. See ADR-0012 for why that was left as it is.
 *
 * Input:  { sessionId: string, paymentIntentId?: string }
 * Result: { success: true, bookingId, pnr, status, confirmedPrice, confirmedCurrency }
...
export interface CreateBookingResult {
    ...
    error?: string;
    /** What the HTTP wrapper should answer. */
    httpStatus?: number;
    [key: string]: any;
}

/**
 * Call this from anywhere in the process. The HTTP route is a wrapper around it.
 */
export async function createBooking(input: CreateBookingInput): Promise<CreateBookingResult> {
    const res = await createBookingResponse(input);
    const payload = await res.json().catch(() => ({ success: false, error: 'Malformed booking result' }));
    return { ...payload, httpStatus: res.status };
}

async function createBookingResponse(input: CreateBookingInput): Promise<NextResponse> {
    const { sessionId, paymentIntentId: piIdFromCaller } = input;
