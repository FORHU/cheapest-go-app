...
## Update — extended to thin-OTV cities

The same principle applies beyond provinces. When a *city's* OTV dest-code is a known
miss, the hotel-code path returns only a partial snapshot scavenged from `hotel_content`
(e.g. Manila: 6 of hundreds). The search now also queries ETG in that path and returns
whichever supplier's set is larger — a swap, not a merge, for the same dedup reason. So
ETG serves any destination OTV can't fully cover, not just provinces.
