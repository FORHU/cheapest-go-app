...
- **`hreflang` over-claims on both domains** — done. Neither domain names a language at a URL where it is not served.
- **`hreflang` becomes cross-domain** — done. Both domains declare the same set of four languages, each at its home: `ko` at `airanggo.com`, `en`/`ja`/`zh` at `cheapestgo.com`, `x-default` at the English page. The homes are a constant in `src/lib/seo/hreflang.ts` rather than an env var, because they are this decision and not configuration; a deployment still emits its own languages as relative paths. The set has to be identical from both sides — Google discards alternates the named page does not confirm — and a test asserts that it is.
