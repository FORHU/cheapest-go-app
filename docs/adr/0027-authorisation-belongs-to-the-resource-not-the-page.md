...
- **A Booking in progress belongs to the account that made it, not just to the browser.** BG-1 came back (2026-09-14) as "can open this booking using other account", and this time it was real, though not a route: checkout keeps the chosen hotel and everything typed into the form in local and session storage, and nothing wiped it, so the next account signed in on the same browser opened checkout onto the previous one's name, email and phone. Signing out now wipes it, and signing in as a different account than the one that made it wipes it (`src/lib/booking/bookingInProgress.ts`). Details typed while signed out are kept for whoever signs in, because that is the funnel this ADR exists to protect.
